...
combine op (Picture f1) (Picture f2) = Picture (\coord -> op (f1 coord) (f2 coord))
...
paintSolid color (Shape shapeF) (Picture baseF) = Picture f
  where f coord | shapeF coord = color
...
paint (Picture patF) (Shape shapeF) (Picture baseF) = Picture f
  where f coord | shapeF coord = patF coord
                | otherwise = baseF coord
------------------------------------------------------------------------------
...
instance Transform Blur where
  apply Blur (Picture f) = Picture g
    where g coord@(Coord x y) = blendColors [f coord, f (Coord (x-1) y), f (Coord (x+1) y), f (Coord x (y-1)), f (Coord x (y+1))]
          blendColors colors = Color (div (sum rs) 5) (div (sum gs) 5) (div (sum bs) 5)
            where rs = map getRed colors
                  gs = map getGreen colors
                  bs = map getBlue colors
------------------------------------------------------------------------------
