...
checkNumber (first, second) = 
  if any isDigit first || any isDigit second
    then Nothing
...
countAndLog predicate xs = do
  let validElements = filter predicate xs
  mapM_ (msg . show) validElements
  return (length validElements)
...
rob from to = 
  balance from +> \amount ->
  withdrawOp from amount +> \withdrawn ->
  depositOp to withdrawn
...
paren c = do
  ...
  case c of
    ...
    ')' -> if count == -1 then put (-1) else put (max (-1) (count - 1))
    _ -> return ()
...
count x = do
  ...
  case lookup x pairs of
    ...
    Just n -> put ((x,n+1) : filter ((/= x) . fst) pairs)
