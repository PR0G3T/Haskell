...
add x (Set xs) = Set (insert x (filter (/= x) xs))
...
data State = Start | EggsAdded | FlourAdded | SugarAdded | BothFlourSugar | Mixed | Error | Finished
  deriving (Eq,Show)
...
step FlourAdded AddSugar = BothFlourSugar
step SugarAdded AddFlour = BothFlourSugar
step BothFlourSugar Mix = Mixed
...
evaluate (Operation "*" left right) = evaluate left * evaluate right
evaluate (Operation _ _ _) = 0  -- default case
